...
const db = mysql.createConnection({
    host: "localhost",
    user: "root",
    password: "tree", // Your MariaDB password
    database: "college"
});
...
app.post('/api/students/add', (req, res) => {
    ...
    db.query(checkQuery, [year, branch], (err, results) => {
        ...
        if (results.length > 0) return res.status(409).json({ message: `Record for ${branch} ${year} already exists!` });
...
app.get('/api/teachers', (req, res) => {
    db.query('SELECT username, name, availability, department, phone FROM Teacher ORDER BY name ASC', (err, results) => {
        if (err) return res.status(500).json({ error: "Database fetch failed" });
        res.json(results);
    });
});
...
app.post('/api/teachers', (req, res) => {
    ...
    db.beginTransaction(err => {
        if (err) return res.status(500).json(err);
        db.query(`INSERT INTO Users (username, role, password) VALUES (?, 'Teacher', ?)`, [username, password], (err) => {
            if (err) return db.rollback(() => res.status(500).json({ message: "Username already exists" }));
            db.query(`INSERT INTO Teacher (username, name, availability, department, phone) VALUES (?, ?, ?, ?, ?)`, [username, name, availability, department, phone], (err) => {
...
app.put('/api/teachers/availability', (req, res) => {
    ...
    db.query(`UPDATE Teacher SET availability = ? WHERE username = ?`, [availability, username], (err) => {
        if (err) return res.status(500).json(err);
